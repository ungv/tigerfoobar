...
<div id="main">
	...
	<div id="banner">
		...
		if ($_GET['page'] == 'server-page' OR $_GET['page'] == 'database-page' OR $_GET['page'] == 'php-page') {
			...
			<div class="block" style="width:180px;padding:0px 0px 0px 0px;">
				...
				<span class='app'><?php echo phpversion() ?><a href='index.php?page=php-page&display=changephpversion' class='settings_link' style="font-size:10px;background-color:#F3F3F3;">change</a></span>
			</div>
			<?php
		}
		?>
		...
		if ($_GET['page'] == "database-page") {
			...
			<div class="settings" style='padding:0px 0px 0px 0px;'>
				...
				<div class='right'>
					...
					<br />
					<a href='index.php?page=database-page&display=changemysqldatadir' class="settings_link"><?php echo $t_settings_change; ?></a><a href='change_datadir_update.php?mode=portable' class="settings_link" style='margin:0px 0px 0px 15px;'>portable mode</a><span class="infobulle_settings"><a class="info" style='margin:0px 0px 0px 0px;' href="#">?<span><?php echo $t_mysql_datadir_portablemode; ?></span></a></span>
				</div>
			...
			if ($_GET['display'] == "mysqlconffile") {
				...
				<div class='menu_display'>
					...
					<pre><?php echo htmlspecialchars(file_get_contents('../conf_files/my.ini', FILE_USE_INCLUDE_PATH)); ?></pre>
				</div>
				<?php
			}
			...
			// DISPLAY : CHANGE DATADIR
			if ($_GET['display'] == "changemysqldatadir") {
				?>
				<div class='menu_display'>
					<h5><?php echo $t_mysql_changedir; ?></h5>

					<div class='changedatadir_frame'>
						<div class="changedatadir">
							<form method="post" action="change_datadir_update.php">
								<ol>
									<li><?php echo $t_changemysqldatadir_1 ?></li>
									<li><?php echo $t_changemysqldatadir_2_1 . ' <span>' . $mysql_datadir . '</span> ' . $t_changemysqldatadir_2_2 ?></li>
									<li><?php echo $t_changemysqldatadir_3 ?>
										<input type="text" name="new_datadir" class="input" style="width:428px" />
										<input type="hidden" name="mode" value="newdatadir" />
										<div style="width:430px;text-align:center;font-weight:normal;padding:5px;margin:10px 0px 0px 0px;border:1px solid #EFCE1D;background-color:#FBD825;color:#895902;-moz-border-radius:2px;-khtml-border-radius:2px;-webkit-border-radius:2px;border-radius:2px;">
											<?php echo $t_changemysqldatadir_warning_conf; ?>
										</div>
					...
					if (count($php_versions) > 1 ){
						...
						foreach ($php_versions as $phpdir) {
							...
							if ($phpversion['status'] == 1) {
								...
								echo '<div class="infobulle_notes"><a href="' . $_SERVER['REQUEST_URI'] . '&editnotes=' . $phpdir . '"class="info">' . $t_settings_notes . '<span>';
								if ($phpversion['notes'] == ''){
									echo '<p style="font-size:9px;padding:0px;margin:0px;text-align:center;color:silver">&laquo; ' . $t_settings_notes_addchange . '</p>';
								}else{
									echo '<p style="padding:10px;">' . $phpversion['notes'] . '</p><p style="font-size:9px;padding:0px;margin:0px;text-align:center;color:silver;">&laquo; ' . $t_settings_notes_addchange . '</p>';
								}
								echo '</a></div><div class="versiondate">' . $phpversion['date'] . '</div>';
								echo '<br style="clear:both"></div>';
							} else {
								echo '<div class="version">';
								echo '<div class="versiontitle_off"><a href="change_php_update_step1.php?oldphpdir=' . $currentphpdir . '&newphpdir=' . $phpdir . '">PHP ' . $phpversion['version'] . '</a></div>';
								echo '<div class="infobulle_notes"><a href="' . $_SERVER['REQUEST_URI'] . '&editnotes=' . $phpdir . '" class="info">' . $t_settings_notes . '<span>';
								if ($phpversion['notes'] == ''){
									echo '<p style="font-size:9px;padding:0px;margin:0px;text-align:center;color:silver">&laquo; ' . $t_settings_notes_addchange . '</p>';
								}else{
							...
							if ($_GET['editnotes'] == $phpdir) {
								...
								<div class="editnotes">
									...
									<form method="post" action="<?php echo $action; ?>">
										...
										<a href="index.php?page=php-page&display=changephpversion"><input type="button" value="<?php echo $t_cancel; ?>" /></a>
									</form>
								</div>
								<?php
							}
	...
	<h3><?php echo $t_portablefiles ?><span class="infobulle_section"><a class="info" href="#">?<span><?php echo $t_portablefiles_intro_1 . str_replace("/","\\", $_SERVER["DOCUMENT_ROOT"]) . $t_portablefiles_intro_2; ?></span></a></span><span style='position:absolute;font-size:11px;margin:3px 0px 0px 20px;padding:0px;font-style:italic;font-weight:normal;color:gray;'><?php echo str_replace("/","\\", $_SERVER["DOCUMENT_ROOT"]); ?></span></h3>
		...
		<div class='localweb_docs'>
			...
			while (key($www_files) !== null){ 
				...
				while (($i < $nbycol) AND (key($www_files) !== null)) {
					echo "<img src='images_easyphp/localweb_doc.gif' width='10' height='10' alt='localweb' /><a href='../" . current($www_files) . "' target='_blank' title='" . current($www_files) . "'>" . cut(current($www_files),25) . "</a><br />";
					next($www_files);
					$i++;
				}
				echo "</div>";
...
<div id="bottom">
	...
	<div class="donation">
		...
		<span class="dollar"><img src='images_easyphp/dollar.png' width='5' height='10' alt='dollar' /></span><a href='index.php?to=donate'><?php echo $donate_title; ?></a>
	</div>

	<div class="footer_bottom">Icons by <a href='http://www.fatcow.com/free-icons' target='_blank'>FatCow</a> | <a href='http://www.easyphp.org' target='_blank'>EasyPHP</a> 2000 - 2012</div>
</div>	
